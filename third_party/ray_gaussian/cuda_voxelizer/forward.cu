	...
	)
{
	...
	// DualRouteGS modification: precomputed covariance does not provide scales.
	// sqrt(trace(Sigma)) is a conservative bound on the largest standard deviation.
	float max_scale;
	if (cov3D_precomp != nullptr)
		max_scale = sqrtf(fmaxf(cov3D[0] + cov3D[3] + cov3D[5], 1e-12f));
	else
	{
	...
	)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK3D_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK3D_SIZE, toDo); j++)
		{
			...
			// DualRouteGS modification: 3-sigma ellipsoidal support, Eq. (41).
            if (power > 0.0f || power < -4.5f)
				continue;

			// float alpha = min(1.0f, opa * exp(power));
			float alpha = opa * exp(power);
			// DualRouteGS: no attenuation-dependent truncation.
